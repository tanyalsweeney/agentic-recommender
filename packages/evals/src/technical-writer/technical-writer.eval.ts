...
describe("Technical Writer eval 9: voice directive — no marketing language", () => {
  it("executive summary contains no banned marketing phrases", async () => {
    const output = await callTechnicalWriterAgent(SEED_MANIFEST, { description: "Document processing pipeline, AWS Lambda, Claude Sonnet" }, upstreamOutputs, DEFAULT_PROVIDER_CONFIGS.technicalWriter);
