...
export const DEFAULT_PROVIDER_CONFIGS: Record<string, ProviderConfig> = {
  ...
  memory_state:            { provider: "anthropic", model: "claude-sonnet-4-6" },
  tool_integration:        { provider: "anthropic", model: "claude-sonnet-4-6" },
  failure_observability:   { provider: "anthropic", model: "claude-sonnet-4-6" },
  trust_control:           { provider: "anthropic", model: "claude-sonnet-4-6" },
  compatibility_validator: { provider: "anthropic", model: "claude-sonnet-4-6" },
  skeptic:                 { provider: "anthropic", model: "claude-sonnet-4-6" },
  technical_writer:        { provider: "anthropic", model: "claude-sonnet-4-6" },
};
