...
import { tenantSecrets, userSecrets, decryptKey } from "@agent12/shared";
...
//
// Resolution order:
// 1. User key: user_secrets row matching (userId, provider). Decrypted via
//    AES-256-GCM using ENCRYPTION_KEY env var (see packages/shared/src/crypto.ts).
// 2. Tenant key: tenant_secrets row matching (tenantId, provider). Same decryption.
// 3. System key: process.env[PROVIDER_REGISTRY[provider].systemApiKeyEnvVar]
