...
| Per-dependency freshness badges | Each dependency carries a date-only badge: `Evaluated [date] ↻`, `Manifest entry`, `Not yet evaluated`, or `Internal`. Click to refresh fires a CV lookup for that dep, charges BYOK, and updates the timestamp. | User requests for freshness are worth supporting without gating. | 2026-05-09 |
...
| Internal dependency referential integrity | Strict at the MCP boundary. Server validates the post-update reference graph; a delete that would leave any internal `dependencies.internal` reference dangling fails with the blocking app ids and displayNames returned in `partialFailures`. Atomic same-call cleanup is supported (delete + upsert in one call validates against the final state). | Catches assistant ref-graph bugs at the call site rather than downstream in Quality Evaluator or Pattern & Cluster Analyzer. Rich error response lets the assistant fix in one retry. Atomic same-call cleanup keeps multi-step coherent edits from being blocked. | 2026-05-12 |
