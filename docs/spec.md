...
3. *Intent gaps.* An explicit checklist of what the code couldn't tell us. Each gap is a structured prompt with three parts: a question, a curated set of common-answer options (check-all-that-apply where multiple answers are valid, single-select where exactly one answer is valid), and an optional free-text field for cases where the user's answer doesn't fit the curated options. Examples:
   - "What's driving this rewrite?" with options [regulatory pressure, cost reduction, capability gap, tech debt, vendor lock-in concern] plus free text
   - "What is the target consolidation goal?" with options [single unified service, smaller set of focused services, replace one specific app, modernization without consolidation] plus free text
   - "What compliance domain does this work fall under?" with options [SOC2, HIPAA, PCI, GDPR, FedRAMP, none] plus free text

   The curated options come from the manifest (admin-managed, evolved over time as common patterns emerge from real user free-text answers). Copilot identifies which intent gap question types apply based on the codebase; the MCP tool response carries the structured prompts so the user's review screen renders them as checkboxes plus optional free text. Users can answer with selected options, free text, or both. If Copilot identifies an intent gap type not yet in the manifest, the question presents as free-text only; the Manifest Gatekeeper workflow promotes frequently-occurring free-text patterns to curated options over time.

   This serves three purposes: cognitive scaffolding so users see what kinds of answers we parse for; cleaner inputs to downstream agents (closed-set values are more reliable to reason against than parsed free text); and speed (users knock out ten intent gaps in under a minute with checkboxes versus several minutes of free-text writing). It is the UX device that operationalizes the code-first-with-refinement pattern: users know what's missing because we tell them, and they know what good answers look like because we suggest them.
...
- Screen labels that need an umbrella (review screen section headers, freshness badges, email templates) are admin-curated `config.ui.string.*` keys. The admin chooses wording per surface.
...
The evaluator returns `qualityScore` (1 to 5) per entry. For inferential fields where the agent's input was missing or insufficient (placeholder text, or a value too generic to distinguish the entry from peers in the same `productCategory`), the evaluator synthesizes a value from the L2 group context and the dependency enrichment chain. When an entry's score is low, the evaluator self-iterates: it generates clarifying questions targeted at the gaps, takes another pass with those questions plus the L2 group context and dependency enrichment chain as additional prompt, attempts to answer the questions from data we already have, and re-scores. Iteration is capped at 3 passes (configurable via the admin dashboard); it ends earlier if scores plateau.
...
**Per-dependency freshness badges:**

The review screen surfaces a small badge next to each dependency in the per-app inventory:

| Dependency state | Badge text |
|---|---|
| Hit in `cv_result_cache` | `Evaluated [date] ↻` |
| Manifest hit, no `cv_result_cache` yet | `Manifest entry, version data at pipeline run` |
| Uncatalogued (manifest miss + `cv_result_cache` miss + not internal) | `Not yet evaluated, research at pipeline run` |
| `isPrivate: true` | `Internal` |

Click to refresh fires the API and web-search calls for that single dependency, updates `cv_result_cache`, charges the user's BYOK, and updates the badge timestamp. There is no cooldown and no value-judgment language about staleness; the date is presented neutrally and the user decides whether to act on it. Refreshing one dependency does not re-trigger the Quality Evaluator (description quality is independent of dependency CV data); the fresh data flows into pipeline-time CV at run submission.
...
Four MCP tools exposed by our server:

| Tool | Purpose |
|---|---|
| `submit_codebase_digest` | Creates a new draft. Accepts the digest payload plus optional raw package manifests. Server normalizes the digest synchronously and returns draft URL plus `status: "evaluating"`. Quality Evaluator runs as a background job; email on completion. |
| `update_codebase_digest` | Modifies an existing draft by ID. Server re-normalizes changed entries and re-runs the evaluator on those entries in the background; email on completion. |
| `get_pending_clarifications` | Returns the list of per-app clarification questions for a draft, scoped to the calling user. |
| `estimate_digest_cost` | Returns an estimated BYOK cost range for a digest payload (or draft ID). No LLM call, no state change. |

Tool responses include forward-looking guidance directing the assistant's next steps. The `submit_codebase_digest` response advises polling `get_pending_clarifications` at a recommended cadence and resubmitting via `update_codebase_digest` until clarifications are addressed. Recommended polling cadences (configurable via admin dashboard): 90 seconds after `submit_codebase_digest` (long initial wait of 10 to 40 minutes); 20 seconds after `update_codebase_digest` (short re-evaluation wait of 30 seconds to 2 minutes). Capable agentic assistants follow the guidance autonomously; less capable ones surface the text for the user to act on.
...
| `codebase_digest_drafts` | Drafts produced by the code-aware intake MCP submission, awaiting user review | id, user_id, tenant_id nullable, digest (jsonb: intake pre-fills, per-app inventory, intent gaps), quality_summary (jsonb: flags, scores, pending clarifications), created_at, expires_at, submitted_at nullable |
...
| Code-aware terminology — internal app vs user-facing word selection | Internal symbols (code, schemas, prompts, design docs) use `app` as the umbrella name for a digest entry. User-facing rendering (Technical Writer, Spec Synthesizer, Plan Synthesizer, Pattern & Cluster Analyzer, Quality Evaluator clarification questions, MCP response guidance) refers to entries by their `displayName` or `productCategory.displayName`. When an umbrella is needed, the agent selects the most semantically accurate word for the specific entry from per-entry signals (`observedPatterns`, `inputs`/`outputs`, `isPrivate`) — examples: "service" for HTTP-shaped, "worker" for batch, "library" for packaged dependency, "utility" for CLI, "app" for application-shaped. The word "tool" is reserved for manifest tools, T&I architectural "tools," and MCP "tools" and is never used to refer to a digest entry. Screen labels are admin-curated `config.ui.string.*` keys. | "Tool" was previously overloaded across four senses (digest entries, manifest tools, T&I architectural tools, MCP tools), causing real cognitive friction in design discussions. The internal rename to `app` removes the overload everywhere we control. The user-facing word-selection rule prevents the umbrella narrowness (apps + services + workers + CLIs + libraries don't fit one English word cleanly) from leaking to readers; agents have the per-entry signal needed to pick the right word per surface. Reserving "tool" globally for senses 2/3/4 preserves established prompt and code language for those senses without renaming the whole CV section. | 2026-05-10 |
| Quality evaluator agent | Two-stage process. Server-side normalization runs synchronously (mechanical extraction from raw package manifests, rule-based inference for pattern fields, LLM-inference fallback for inferential essentials when the agent did not provide them). LLM evaluator runs as a background BullMQ worker job with a 3-layer prompt cache (system + rubric, full per-app inventory + tenant context, per-app entry with dependency enrichment chain). Evaluator scores per-app entries (1 to 5), synthesizes missing inferential fields from the L2 group context, and self-iterates up to 3 passes (configurable) on low-scored entries by re-prompting itself with clarifying questions over existing data. | Quality variance is the central risk in code-aware intake (we never see source, can't validate description accuracy). Self-iteration over data we already have replaces user-mediated clarification for everything the evaluator can resolve from existing context; the user is not asked questions the agent could have answered from code. The 3-layer cache pattern keeps per-call cost economic across large digests. Synthesizing missing fields rather than gating on completeness reflects the design objective: produce an excellent migration spec and plan, not a complete tabular form. | 2026-05-06, updated 2026-05-09 |
...
| Pattern & Cluster Analyzer | Dedicated agent for code-aware digests, runs sequentially after Quality Evaluator as a background BullMQ worker job. Groups apps by `productCategory`, makes one LLM call per multi-app category in parallel (single-app categories skip the LLM call and emit `opportunity: 'none'` deterministically). 3-layer prompt cache (system + rubric, full per-app inventory + tenant context, per-category subset). Output: `consolidationOpportunity`, structured `clusters` / `blockers` / `capabilityVariance` / `supportingEvidence`, `clarificationQuestions` in intent-gap format, and the `consolidationStrategyQuestion` per multi-app category. Stored in dedicated `cluster_analysis` jsonb column on `codebase_digest_drafts`. On update, only affected categories re-run. | Cluster analysis at the `productCategory` (app/product) level matches how users think about migration: consolidate apps that do similar things. Sub-component-level analysis would suggest absurdities like "consolidate auth services across products" that nobody actually wants. Targeted clarification surfaces context that would not otherwise be captured (regulatory, performance, contractual reasons that drove divergent choices). Per-category LLM calls in parallel (skipping single-app) keeps cost economic; sequential placement after Quality Evaluator means the analyzer sees synthesized fields. Always running the analysis is cheap; surfacing depends on user intent and assessment magnitude. | 2026-05-06, updated 2026-05-09 |
| Consolidation analysis Pass 1 surfacing | Three modes based on user intent and assessment magnitude: Full (prose for simple cases under ~5 entries to one target; compact table for complex cases with multiple groupings or mixed outcomes), Brief (single-sentence callout when user did not request guidance and gains are marginal), Omitted (no Pass 1 surface when no opportunity exists). Pass 2 spec doc carries the section whenever the digest has 2+ apps (renders full analysis when opportunities exist, brief "assessed, none found" statement when it ran but no opportunity surfaced; omitted entirely for single-app digests). Both prose and table forms work without the architecture diagram. | Consolidation is the load-bearing decision in migration scenarios; the surface form must communicate it clearly to free-tier users (who do not see the diagram) and respect exec-summary length. Prose handles simple cases elegantly; structured table handles complex cases compactly without sacrificing clarity. Omitting Pass 1 mention when there is no real opportunity prevents the recommendation from being trained-out as noise. Pass 2 still shows the user we looked (multi-app case), but only when there was something to look at (single-app digests omit). | 2026-05-06, updated 2026-05-10 |
| Code-aware per-spec+plan pricing | Code-Aware Pass 2 priced per spec+plan deliverable ($199 each, BYOK required), at parity per target with text-intake Pass 2; customer-selectable subset from the consolidation analysis target set. Code-Aware Pass 1 priced at $49/run (BYOK required) and includes consolidation summary, target count, Pass 2 pricing math, and migration orchestration overview. Pass 2 invocations (Spec Synthesizer + Plan Synthesizer) run only for selected targets. | A multi-target code-aware run produces N independently-executable spec+plan pairs; flat per-run pricing undercounts deliverable value. Per-spec+plan pricing aligns price with units of executable work and lets customer scope their purchase. Per-target parity with text-intake Pass 2 keeps the pricing model uniform and easy to communicate ("$199 per target deliverable, regardless of intake method"); the value differential of code-aware deliverables (grounded in actual code, includes migration mapping) is not currently priced as a premium because there is no production data yet to validate a premium. The provisional pricing principle applies: code-aware can be repriced upward once data confirms the value differential. Code-Aware Pass 1 mirrors text-intake Pass 1 structure: a lower-commitment way to validate the architectural recommendation before paying per-target for Pass 2 deliverables. Migration orchestration overview is bundled with Pass 1 because cross-target sequencing is independent of which targets the customer ultimately implements. | 2026-05-07 |
| Provisional pricing | All initial prices in the pricing table are estimates pending production cost data. After launch, real per-run costs from the agent call log will be used to validate and adjust pricing. Prices are admin-configurable thresholds and can be adjusted without a deploy. | Without production cost data, pricing is necessarily estimated. Treating initial prices as provisional and committing to revisit them with real data is more honest than pretending the estimates are optimized. Admin-configurability means revisions are a config change, not a deploy. The Run Pack pricing validation TODO captured this principle for one tier; this row generalizes it. | 2026-05-07 |
| MCP integration model | User's AI assistant is their existing IDE tool (Copilot, Claude Code, Cursor, etc.). We control tool definitions and response payloads; we do not control the assistant's model, behavior, or how it surfaces our responses to the user. | Standard MCP protocol semantics: server responds when called, cannot push to clients. Architecting around this boundary keeps the design honest about what we can guarantee. Quality of digest is bounded by quality of assistant; clarification round-trips are best-effort and user-initiated. | 2026-05-09 |
...
| BYOK cost transparency | Public reference page (accessible pre-auth) with typical per-app token usage ranges and per-provider rates (with "effective as of" date stamp). The `estimate_digest_cost` MCP tool returns dynamic estimates for assistants that want them. No cost confirmation gate at submit. | Users paying BYOK should know the rough math before committing. Static reference covers occasional users; the endpoint covers integrated assistants that can compute and display estimates inline. Avoiding a confirmation gate keeps the submit flow simple and matches the principle that BYOK costs are bounded by what is in the user's API account anyway. | 2026-05-09 |
| Per-dependency freshness badges | Each dependency on the review screen carries a neutral badge: `Evaluated [date] ↻`, `Manifest entry`, `Not yet evaluated`, or `Internal tool`. Click to refresh fires CV API and web-search calls for that single dependency, charges BYOK, updates the badge timestamp. | Date-only framing avoids editorializing about staleness (which implies a value judgment we do not own). User decides whether to act on the date. Per-dep refresh provides agency without forcing decisions; refreshing one dep does not re-trigger Quality Evaluator (description quality is independent of CV data freshness). | 2026-05-09 |
| MCP server hosting | Dedicated service in the monorepo at `packages/mcp/`, deployed to Railway (or equivalent long-running host like Fly.io); not on Vercel. HTTP+SSE transport per MCP standard for cloud-hosted servers. Shares dependencies on `packages/shared/` and `packages/agents/`; uses same Redis as `packages/workers/` for BullMQ enqueue. Does not depend on `packages/web/`. | Pre-launch decision committing to Option B avoids customer-affecting URL migration later. First user is expected to use MCP, so any extraction work after launch would have customer impact. Vercel ruled out because serverless function model fits poorly with HTTP+SSE patterns and longer-lived response shapes. Total cost (2-3 days upfront) lower than Option A (start in Next.js then extract: 5-10 days plus customer migration window). | 2026-05-10 |
| MCP authentication mechanics | Bearer token in HTTP header (`Authorization: Bearer agent12_pat_<32 bytes base64url>`). Server stores SHA-256 hashes in `user_api_tokens.token_hash`; never persists raw tokens. Soft-delete revocation via `revoked_at` (audit trail). Per-token rate limit defaults to 60 requests per minute, configurable via admin dashboard, enforced via Redis. Debounced `last_used_at` updates (once per ~60s per token). On miss: 401 with generic error. | Token format follows GitHub PAT pattern (`agent12_pat_` prefix) for easy secret-scanner identification and namespace reservation for future token types. SHA-256 sufficient for high-entropy tokens we generate ourselves; bcrypt/argon2 overkill for tokens (their slow-hash benefit applies to low-entropy passwords, not 256-bit random tokens). Soft-delete preserves audit trail. Rate limit defaults loose (steady-state polling under 4 RPM) but bound runaway integrations. Generic 401 message avoids leaking validation details. | 2026-05-10 |
| MCP response-driven iteration pattern | Tool responses include forward-looking guidance directing the assistant's next steps (Pattern 1). Server cannot push to MCP clients (standard MCP semantics), so iteration relies on capable agentic assistants reading our response payloads and continuing autonomously. Polling cadence configurable via admin dashboard, split by context: 90s default after `submit_codebase_digest` (long initial wait of 10-40 minutes); 20s default after `update_codebase_digest` (short re-eval wait of 30s-2min for changed entries). Long-polling pattern explicitly rejected. | Pattern enables autonomous-after-initial-invocation iteration without requiring user to click through clarification prompts in chat. Capable assistants (Claude Code, modern Cursor agent mode) loop on response guidance; less capable ones surface response text to user. Split cadence reflects different wait shapes (initial vs re-eval); 15-second polling on the long initial wait would burn ~100 polls per digest with no responsiveness gain since nothing changes for tens of minutes. Long-polling rejected because Quality Evaluator wait (10-40 min) exceeds Vercel function timeouts and connection lifetimes; would require chained calls (just polling with extra ceremony) and dedicated long-running infrastructure. | 2026-05-10 |
