...
| Code-aware digest implicit dedup window | 5 minutes | Tier 2 implicit-dedup TTL for `submit_codebase_digest` and `revise_codebase_digest`. Per-tenant configurable. |
...
**Audience:** Teams with an existing system (typically a multi-app internal landscape) who want a recommendation for a target architecture, often a consolidation or migration target. A team running 20 loosely associated internal apps and looking to consolidate them into a unified agentic system is the canonical example.

**MCP integration model:**

The user's AI assistant is their existing IDE tool (GitHub Copilot, Claude Code, Cursor, Continue.dev, or any other MCP-aware assistant). It runs in their environment, with their LLM credentials, against their codebase. We do not deploy, configure, or control the assistant. We communicate via the standard MCP protocol: the assistant is a client, our server is a server, the assistant calls our tools when invoked, and we respond.

We control our tool definitions (name, description, input and output schema), our response payloads (structured data plus natural-language guidance), and our public docs. We do not control the assistant's model, system prompt, tool-calling logic, depth of codebase analysis, or how it surfaces our responses to the user.

This boundary shapes design decisions throughout the Code-aware intake flow. Clarification loops are best-effort: a capable agentic assistant loops automatically on rework feedback; a less-capable one prints our response in the chat and waits for the user. Quality of digest is bounded by quality of assistant. Our influence is the tool description and response payload; everything downstream is the assistant's call.

**Architecture:**

The user runs Copilot Chat (or equivalent) in their IDE with the codebase open. They invoke our MCP `submit_codebase_digest` tool. Their AI assistant reads the codebase using its own context (which it already has access to), produces a structured digest matching our schema, and submits via the MCP tool. Our server creates a draft and runs the Quality Evaluator in the background. When eval completes, the user receives an email with the draft URL. They open it, review and refine via our standard review screen, submit to the pipeline.
...
The digest is a structured document with one primary component: per-app inventory. Project-level intent is collected separately ([Pre-digest intent collection](#pre-digest-intent-collection)) and is not part of the digest payload; Wave 1+ agents derive project-level signals (deployed platform, dominant language, observed scale signals, external integrations) from the per-app inventory directly rather than from pre-baked intake-step values.

1. *Per-app inventory.* One structured entry per discovered app. For purposes of the digest, an app is a discrete product or application within the codebase (e.g., FinanceLaunchRisk, ContractVolumePredictor), not an internal sub-component (an auth-service inside an app, a data access layer). Internal sub-components are described as part of the app's entry (in `dependencies` and `distinguishingCharacteristics`), not as separate entries. The migration unit is the app, not the sub-component. Each entry carries:
   - `id`: unique within the digest, auto-generated. Allows disambiguation when multiple apps share the same display name (a codebase routinely has both `apps/finance-launch-risk-v1/` and `apps/finance-launch-risk-v2/` reflecting different generations of the same product family) and stable cross-reference via internal dependencies.
   - `displayName`: what the user calls it.
   - `path`: location in the codebase, helps disambiguate.
   - `productCategory`: structured category this app belongs to (`name`: machine-readable identifier like "launch-risk-evaluation"; `displayName`: human-readable like "Launch Risk Evaluation"). Groups similar-function apps together for cluster analysis and consolidation reasoning. A category may contain one app (a unique-purpose app) or many (multiple Launch Risk Evaluators that may be candidates for consolidation).
   - `primaryPurpose`: concrete description of what THIS app does, not a category label ("handles risk evaluation"). Example output: "evaluates upcoming product launches against historical launch profiles to surface risk patterns observed in similar past launches."
   - `language`, `framework`.
   ...
   - `externalIntegrations`: third-party services this app calls.
   - `observedPatterns`: sync API, async queue, event-driven, batch worker, etc.
   - `distinguishingCharacteristics`: what makes THIS instance unique, mechanism-level. The hardest reasoning the digest production has to do. Example output: "uses a custom XGBoost regression trained on 5 years of internal launch data; integrates with Salesforce for launch metadata; runs nightly batch via Airflow rather than per-request."
   - `isPrivate`: true for internal-only apps, false for public packages. Defaults to true; user confirms public during review.
   - `currentDecisionMaking`: where judgment, scoring, or threshold-branching happens in this app today (scoring functions, decision trees, manual approval workflows, threshold-based logic). The agentic-upgrade opportunity per app; load-bearing for Wave 1+ recommendations about where to add agency. Example output: "hard-coded score thresholds in `risk_scorer.py` classify each launch as low / medium / high; admin UI lets the launch manager bump the score before it reaches the approval queue."
   - `humanInTheLoop`: where humans currently gate, approve, or verify in this app's flow (approval queues, manual triage hooks, scheduled human review steps). Trust & Control agent's input for HITL recommendations. Example output: "launch manager must approve or reject every high-risk classification in the admin queue before the result is surfaced to the launch team; no auto-approval path."
...
**Pre-digest intent collection:**

Two structured questions plus an optional free-text constraints field are collected before the assistant produces the digest. None of these are inferrable from code; all flow into `verifiedContext` and reach Wave 1+ agents as part of their input.

*Target topology* (single-select, plus free text). Which shape do you want for the upgraded architecture? Curated options:

- *Augmentation*: keep existing apps, add an agentic layer over them.
- *1:1 replacement*: replace each app with a modernized agentic version, no consolidation.
- *Consolidation to a single target if possible*: collapse to one unified agentic system. Fallback to multiple targets if blockers (compliance, contractual, technical) prevent collapsing fully; Pattern & Cluster Analyzer surfaces the blockers post-digest.
- *Consolidation to multiple smaller targets*: collapse into a smaller number of agentic systems. Pattern & Cluster Analyzer recommends the grouping per category post-digest.
- *Recommend the best fit*: assess the codebase and propose the most appropriate shape.

*Timeline pressure* (single-select, plus free text). Curated options: no fixed deadline, soft target, hard deadline within 6 months, hard deadline within 6 weeks.

The optional `constraints` field captures hard requirements that should narrow the recommendation set ("must stay on AWS," "team only knows Kafka," "no LangGraph"). Constraints are classified at submission time as binary exclusions or optimization targets, identical to text-intake constraints handling ([Intake flow](#intake-flow)).

*Bimodal collection.* The user reaches the same end state through either the web UI (sign in, answer the questions, receive an assistant snippet that references the pre-digest answers by id) or the MCP payload itself (assistant collects the answers in chat and includes them as optional fields on `submit_codebase_digest`). The server unifies both paths into the same draft state. Idempotency rules apply identically across paths.

*Catalog and Gatekeeper.* Questions and option lists live in `manifest_intent_gap_questions`. The Manifest Gatekeeper promotes frequently-occurring free-text answers to curated options over time. The `owner` field follows the standard tenant-override pattern; MVP ships with `owner = 'global'` only.

*Why pre-digest, not post-digest.* Digest production is pure inventory (existing settled decision). Intent does not affect what the assistant emits, so collecting it earlier is cleaner: Quality Evaluator and Pattern & Cluster Analyzer reason with intent during their background passes, and the review screen stays focused on validating digest accuracy rather than answering project-level questions.

**Terminology — internal "app" vs user-facing word selection:**

"App" is the internal name for a digest entry throughout code, schemas, prompts, design docs, and admin surfaces. The umbrella is deliberately broad: the bucket includes applications, services, batch workers, CLIs, utilities, and internal libraries. No single English word fits every shape cleanly.

User-facing rendering rule (applies to every agent that produces output a user reads — Technical Writer, Spec Synthesizer, Plan Synthesizer, Pattern & Cluster Analyzer, Quality Evaluator clarification questions, MCP tool response guidance):
- Prefer the entry's `displayName` or its `productCategory.displayName`. Most surfaces need no umbrella term at all.
- When an umbrella is needed, select the most semantically accurate word for the specific entry from per-entry signals (`observedPatterns`, `inputs`/`outputs`, `isPrivate`). Examples: an HTTP-shaped entry is a "service"; a scheduled batch entry is a "worker"; a packaged dependency is a "library"; an application-shaped entry is an "app"; a CLI entry is a "utility."
- The word "tool" is reserved for manifest tools, the Tool & Integration agent's architectural "tools," and MCP "tools." It is never used to refer to a digest entry.
- Screen labels that need an umbrella (review screen section headers, email templates) are admin-curated `config.ui.string.*` keys. The admin chooses wording per surface.

Agent system prompts carry this rule explicitly so the contextual word selection happens reliably across rendering layers.

**Quality evaluation and clarification loop:**

After digest production, the system normalizes the digest and assesses its quality before the digest reaches the user's review screen. The aim is an excellent migration spec and plan, not a complete tabular form. Mechanically extractable data (dependencies from raw package manifests, language and framework from manifests) is parsed server-side and treated as authoritative. Pattern fields (`observedPatterns`, parts of `externalIntegrations`) are derived from the parsed dependency set using rule-based inference. Inferential essentials (`displayName`, `primaryPurpose`, `productCategory`) come from the AI assistant when provided; the server fills missing or placeholder values from dependencies, README, and file tree.

An LLM evaluator (the Quality Evaluator agent) runs on the assembled digest as a background BullMQ worker job, not in the MCP request path. It uses a three-layer prompt cache structure: L1 carries the system prompt and scoring rubric (cached); L2 carries the full per-app inventory of all N apps in the digest plus the tenant communication context if applicable (cached); L3 carries the specific per-app entry being evaluated, with its dependencies enriched in order from manifest hits (curated metadata), `cv_result_cache` hits (current CVE, EOL, pricing snapshot), internal cross-references (sibling digest entries referenced by id), or marked uncatalogued. The digest-time enrichment chain skips fresh API research; that is CV's job at pipeline time.

The evaluator returns `qualityScore` (1 to 5) per entry. For inferential fields where the agent's input was missing or insufficient (placeholder text, or a value too generic to distinguish the entry from peers in the same `productCategory`), the evaluator synthesizes a value from the L2 group context and the dependency enrichment chain. When an entry's score is below the configurable sufficiency threshold (default 3 out of 5), the evaluator self-iterates: it generates clarifying questions targeted at the gaps, filters them to those whose resolution would shift `qualityScore` toward sufficiency (questions about non-load-bearing fields, or improvements that would land within the same scoring bucket, are dropped at generation time), and takes another pass with the filtered set plus the L2 group context and dependency enrichment chain as additional prompt. It re-scores after each pass. Four early-exit conditions: 3-pass cap (configurable), score reaches sufficiency threshold, score plateau, or filtered question set is empty. The empty-set case signals that the remaining gaps require user input rather than more self-attempt; those questions carry forward as `pendingClarifications` per below. The filtered iteration follows the Outcome-gated execution principle (see Settled decisions).

The review screen presents the best version of each entry the evaluator could produce, with the per-entry `qualityScore` shown. For any entry where the evaluator still has open `clarifyingQuestions` after self-iteration (the answer is information the evaluator could not derive from data we have), the screen surfaces an inline option to delegate the questions to the user's AI assistant. When the user invokes their assistant for this purpose, the assistant calls `get_pending_clarifications` for the entry, examines the code, and resubmits richer data via `update_codebase_digest`; the evaluator then re-runs on the changed entry. The user can also edit any field manually for clarity. The user submits the draft to the pipeline when satisfied; there is no quality bar that gates submission.

**Pattern & Cluster Analyzer:**

A dedicated agent runs after digest production and examines the per-app inventory at the `productCategory` level to identify consolidation opportunities.

The analyzer runs as a background BullMQ worker job, sequentially after the Quality Evaluator completes (so it sees any synthesized fields from the Quality Evaluator's group-context pass). For each multi-app category, the analyzer makes one LLM call; calls run in parallel with a configurable concurrency cap. Single-app categories skip the LLM call entirely; the server emits `consolidationOpportunity: 'none'` deterministically. Each per-category call uses a 3-layer prompt cache: L1 carries the system prompt, analyzer instructions, scoring rubric, and output schema (cached); L2 carries the full per-app inventory of all N apps in the digest plus tenant communication context if applicable (cached); L3 carries the specific category being analyzed and the subset of per-app entries within it (no cache).

The analyzer's work, per category:

- *Categorize.* Groups apps by `productCategory`. A category may contain one app (unique-purpose) or many (multiple apps serving similar functions, candidates for consolidation analysis).
- *Cluster within categories.* For categories with multiple apps, identifies clusters of similar capability and notes capability variance (do all 5 Launch Risk Evaluators have similar capabilities, or do some require more robustness for compliance, scale, or feature reasons?). Captures historical context where observable (e.g., one cluster uses a newer technology stack reflecting later development).
- *Assess consolidation opportunity.* Produces an assessment per category: `substantial` / `marginal` / `none`. Heuristics consider count of consolidatable apps, estimated complexity reduction, capability gains, and identified blockers (compliance boundaries, contractual obligations, technical incompatibilities).
- *Generate targeted clarification questions.* Per the user-facing word selection rule above, the analyzer renders these in the structured intent-gap format using each entry's `displayName` and `productCategory.displayName`. Examples (canonical phrasing — actual word choice for the umbrella is per-entry, agent-selected):
   - "We see 5 Launch Risk Evaluators. 4 use a basic statistical model; 1 uses an ML-based approach. Was this driven by:" [check all that apply: compliance requirement, performance constraint, accuracy requirement, team preference, contract obligation, available training data] + free text
   - "Your codebase contains 4 Contract Volume Predictors serving different lines of business. Are the differences between them:" [check all that apply: business-line-specific data sources, business-line-specific approval workflows, regulatory differences, team ownership] + free text
- *Generate consolidation strategy intent question.* For each category with multiple apps, the analyzer surfaces the user's consolidation intent as a structured question phrased around the category's `displayName` (e.g., "What is your intended consolidation approach for the {N} Launch Risk Evaluators?"). Options: [1-to-1 modernization (replace each with a modern equivalent, no consolidation); full consolidation (collapse into a single unified system); group by business area; group by complexity / capability tier; group by team ownership; recommend one based on what you see] + free text. The user's answer flows into the pipeline as part of the verified context.

The per-category output is structured:

| Field | Shape | Purpose |
|---|---|---|
| `productCategory` | object: name + displayName | Identifies the category being analyzed |
| `consolidationOpportunity` | enum: `substantial` / `marginal` / `none` | Assessment of consolidation value |
| `summary` | string | One-sentence rationale for the consolidation recommendation, suitable for inline display in row-summary UIs (e.g., spreadsheet review screen). Distinct from `reasoning`, which is the longer-form explanation. |
| `reasoning` | string | Free-text explanation covering capability variance, identified blockers, historical context |
| `clusters` | array of `{label, description, appIds}` | Within-category groupings |
| `capabilityVariance` | string or null | Description of differences across clusters; null if single cluster or no meaningful variance |
| `blockers` | array of `{type, description, affectedAppIds}` | Identified blockers (compliance / contractual / technical); empty if none |
| `supportingEvidence` | array of `{appIds, claim}` | Pointers from reasoning back to specific entries |
| `clarificationQuestions` | array of intent-gap-format questions | Targeted questions surfaced when meaningful divergences are found |
| `consolidationStrategyQuestion` | intent-gap-format question with fixed options | The user-intent meta-question per multi-app category |

All output is stored on the draft in a dedicated `cluster_analysis` jsonb column on `codebase_digest_drafts`.

On `update_codebase_digest`, the analyzer re-runs only on affected categories (categories whose membership changed because an entry was edited within them, added, removed, or had its `productCategory` changed). Categories with unchanged membership reuse their prior analysis.

The analyzer's questions surface as additional intent gaps on the user's review screen, alongside the standard intent gaps from digest production. They are scoped to divergences whose resolution would shift the consolidation recommendation; categories with no such divergence do not generate questions (Outcome-gated execution principle, see Settled decisions).
...
**Consolidation analysis Pass 1 report:**
...
- **Full surfacing**: when the user explicitly requested consolidation guidance OR the assessment is `substantial`. Two rendering forms based on the structural complexity of the recommendation. In both forms the agent follows the user-facing word selection rule — entries referred to by `displayName`, umbrella terms chosen per-entry when needed:
   - *Prose*: when the recommendation is a single consolidation grouping with a short "from" list (typically under ~5 entries collapsing into one target). Two sentences at most. Example: "We propose consolidating the 5 Launch Risk Evaluators into RiskAgent, a single unified system that handles risk evaluation across all product lines."
   - *Table*: when the recommendation has multiple consolidation groupings, mixed outcomes (consolidate some, retain others), or long "from" lists. Three columns: current entries, target system, rationale. Compact, scannable, conveys structure without requiring visual support. Column headers are admin-curated `config.ui.string.*` keys.

- **Brief surfacing**: when the user did not request guidance AND the assessment is `marginal`. Single-sentence callout, agent selects the umbrella to fit the inventory shape. Example: "We investigated consolidation opportunities across your inventory; gains assessed as marginal. Detailed analysis in Pass 2 spec doc."
...
Pass 2 Consolidation Strategy section is included whenever the digest contains 2 or more apps. When opportunities were found (`substantial` or `marginal` on any category), the section renders the full analysis: per-category cluster analysis, capability variance, target topology rationale, tradeoffs considered. When the analysis ran but no opportunities surfaced (every category assessed `none`), the section renders a brief statement that the inventory was assessed for consolidation and no opportunities were identified — the user sees that we looked. Single-app digests omit the section entirely; there was nothing to assess. Pass 2 plan doc structures phases around the consolidation when it applies: per-phase migration mapping (which existing apps merge into what, which retain, which retire), dependencies between phases.

**Migration orchestration overview:**

When the consolidation analysis identifies multiple target systems, Pass 1 includes a migration orchestration overview describing how the target systems relate to each other and a suggested cross-target sequence. The overview covers: shared-service identification (do RiskAgent and ContractAgent both depend on a common identity service that should be built first?), critical-path target systems (which target's completion unblocks others?), suggested migration order across the target set, and risk callouts for cross-target dependencies. The overview is a small document (typically a paragraph plus a dependency graph) that supplements the per-target spec+plan deliverables.

Pass 2 produces N spec+plan pairs for code-aware runs, one per target system, with the customer selecting which targets to purchase. The Plan Synthesizer and Spec Synthesizer (with the migration-mapping specialist prompt) run once per selected target system. Each spec+plan pair is independently executable: the spec describes the full target system architecture; the plan phases the build for that target including the migration mapping for existing apps that merge into it. The migration orchestration overview from Pass 1 sits above the per-target plans, providing the cross-target sequencing context.

**Per-target Pass 2 invocation:**

Each target system identified in Pass 1's consolidation analysis is its own Pass 2 purchase: independently selected, independently billed, independently delivered. A user buying Pass 2 for RiskAgent triggers one Pass 2 run scoped to RiskAgent's apps; a separate purchase later for ContractAgent triggers another run scoped to ContractAgent's apps. Per-target runs link back to their originating code-aware Pass 1 via `runs.parent_run_id`.

The per-target run internally re-pipelines Waves 1-3 + Skeptic on the live digest filtered to the target's current app membership, then renders the Pass 2 deliverable (six architecture-domain spec sections + Migration Mapping section + phased plan). The user sees "Pass 2 running" without the internal re-pipeline being surfaced.

*Why re-pipeline rather than render against shared upstream.* Per-target reasoning produces sharper recommendations than per-target rendering of shared upstream. Each target's spec describes its specific architecture, components, and trust boundaries, not boilerplate shared across all targets.

*Pattern & Cluster Analyzer bypass on re-pipeline.* The consolidation grouping was decided and confirmed at parent Pass 1 time; PCA doesn't re-run during the re-pipeline. The per-target subset is filtered from parent's existing PCA output and fed to Waves 1-3 as context. Avoids the awkwardness of PCA producing a different consolidation verdict on the subset than it did on the parent (subset clustering can differ from full-inventory clustering).

*Cross-target context handling.* Parent Pass 1's migration orchestration overview supplies cross-target context (shared services, critical-path targets, suggested order). If the user has revised the digest since parent Pass 1 in ways that affect cross-target dependency boundaries (additions, or removals that flip whether a service crosses target boundaries), the overview auto-refreshes before the per-target re-pipeline runs. The refresh is one Technical Writer-equivalent call on BYOK, no additional service fee. The Pass 2 deliverable carries an annotation: "based on cross-target analysis as of [timestamp]."

*Live target list for buy affordances.* Pass 2 buy buttons reflect the current target list (derived from current PCA output + user strategy answers), not the target list frozen in the parent Pass 1 deliverable. Parent Pass 1 deliverable stays as a historical artifact showing what was originally recommended; per-target Pass 2 deliverables reflect current intent.

*Per-target Skeptic.* Skeptic re-runs as part of the per-target re-pipeline on the subset. Skeptic may surface caveats the parent Pass 1 didn't carry (e.g., compliance issues that only emerge when isolating these specific apps) or omit caveats that don't apply to the subset.

**Multi-part submission and deferred eval:**

`final` defaults to `true` on both `submit_codebase_digest` and `revise_codebase_digest`, so single-shot assistants do nothing special. Multi-part assistants explicitly override to `final: false` on the initial submit and on every intermediate revise, then accept the default on the final revise. While `final` resolves to `false`, draft status stays `intake-in-progress` and the server applies edits to draft state without enqueuing eval. The first resolved `final: true` flips status to `evaluating` and enqueues Quality Evaluator and Pattern & Cluster Analyzer against the complete inventory. Multi-part is recommended when the inventory has many apps (typically >10), per-app analysis is rich (capturing `currentDecisionMaking` and `humanInTheLoop` at depth in addition to identification and interface metadata), or assistant context-window pressure favors splitting.

*Idle-timeout backstop.* If no `revise_codebase_digest` call (content-bearing or carrying resolved `final: true`) arrives within `multi_part_idle_timeout_minutes` (default 30, admin-configurable), the server flips status to `evaluating` automatically and enqueues eval. The completion email notes that intake may have been cut short.

**MCP tool surface:**

Six MCP tools exposed by our server. Tools split into two control patterns. **Server-driven** (`get_pending_clarifications` + `update_codebase_digest`) covers the dominant Quality Evaluator clarification loop: server identifies gaps, ships request shapes, assistant fills in exactly what was asked. **Assistant-driven** (`revise_codebase_digest`) is the escape hatch for changes the server cannot template ahead of time, such as new app discovery or freeform field edits. `get_codebase_draft` and `estimate_digest_cost` are read-only utilities.
...
- `submit_codebase_digest` (entry point). **Input:** `digest` (containing `appsToCreate` as array of `AppEntryCreate`, optional `intentGapsToCreate` as array of `IntentGapCreate`, optional `intakePrefills` as `IntakePrefills`), optional `final` (boolean, defaults to `true`; set `false` for multi-part intake), plus optional `idempotencyKey` (see Idempotency subsection below). Pre-digest intent fields are accepted inline per [Pre-digest intent collection](#pre-digest-intent-collection); this envelope is updated to enumerate them in a follow-up PR. **Output:** `draftId`, `draftUrl` (for assistant introspection only; the user's first sight of the URL is the completion email), `status` enum: `evaluating` (default, when `final` resolves to `true`) or `intake-in-progress` (when `final: false`), `assignedAppIds`, `assignedIntentGapIds`, `expiresAt` (iso8601), `pollGuidance`, `dedupedFromRecentSubmit` (boolean), `dedupePath` (enum: `explicit`, `implicit`, or null when not deduped), `isError`, `partialFailures`.
...
- `revise_codebase_digest`. **Input:** `draftId`, optional `final` (boolean, defaults to `true`; set `false` for intermediate multi-part revises), optional `apps.{create, update, delete}`, optional `intentGaps.{create, update, delete}`, optional `intakePrefills` (partial-merge), plus optional `idempotencyKey` (see Idempotency subsection below). Empty body is valid when resolved `final` is `true` (default or explicit); empty body with explicit `final: false` returns `isError: true`. **Output:** `assignedAppIds`, `assignedIntentGapIds`, `affectedEntries`, `affectedCategories`, `status` enum: `intake-in-progress` or `evaluating`, `danglingReferences` (array of `{id, displayName, referencePath}`, advisory per existing referential integrity decision), `pollGuidance`, `dedupedFromRecentSubmit` (boolean), `dedupePath` (enum: `explicit`, `implicit`, or null when not deduped), `isError`, `partialFailures`.
...
| `currentDecisionMaking` | string or null | Where judgment / scoring / threshold-branching happens in this app today |
| `humanInTheLoop` | string or null | Where humans currently gate, approve, or verify in this app's flow |
...
| `code` | enum | One of `unknown_field`, `type_mismatch`, `missing_required`, `outside_template` (`update_codebase_digest` only), `invalid_value`, `duplicate_id`, `id_not_found`, `idempotency_collision` (`submit_codebase_digest` and `revise_codebase_digest`) |
...
*Idempotency (`submit_codebase_digest` and `revise_codebase_digest`):*

Two-tier dedup, mode determined by whether the assistant supplies an `idempotencyKey`. Both tiers compute the same `request_hash`: deterministic JSON canonicalization of the input payload followed by SHA-256. Cache key scoping differs by tool: submit uses `{user_id, key-or-hash}`; revise uses `{user_id, draftId, key-or-hash}` so retries on different drafts cannot collide.

*Tier 1: explicit (assistant supplies `idempotencyKey`).* Redis with a 24-hour TTL.

- Same key + identical payload: server returns the cached response with `dedupedFromRecentSubmit: true` and `dedupePath: "explicit"`. No new state change, no new background evaluation, no additional BYOK billing.
- Same key + different payload: server returns a `partialFailures` entry with `code: "idempotency_collision"` and `isError: true`. The error message instructs the assistant to generate a fresh key for a new intentional call. Indicates an assistant key-management bug, not user intent.
- Fresh key: server processes the call normally and stores the entry. Tier 2 is bypassed on keyed calls; the explicit key is the assistant's signal that it wants precise retry semantics.

*Tier 2: implicit (assistant supplies no `idempotencyKey`).* Redis with a short TTL (`codebase_digest_drafts.implicit_dedup_window_minutes`, default 5, per-tenant configurable).

- Payload hash matches a recent same-user (and, for revise, same-draft) call within the window: server returns the cached response with `dedupedFromRecentSubmit: true` and `dedupePath: "implicit"`. Catches the dominant naive-retry failure mode (assistant retries after a network blip without supplying a key) at essentially zero infrastructure cost. Multi-part submission makes this protection load-bearing on revise: each part is a separate billable call, and a network-retry double-create would silently corrupt the inventory.
- No match: server processes the call normally and stores the implicit cache entry.

*Override path for forcing a fresh call.* Supply a fresh `idempotencyKey` to bypass Tier 2. The call processes normally; the implicit cache is not consulted. Explicit always beats implicit.

Implemented as a Redis-backed helper in `packages/mcp/src/idempotency.ts`. Tool description language recommends assistants generate a fresh uuidv7 per user-initiated call and reuse it only on retries of that same call.

Each non-cached submit triggers a fresh background evaluation billed against the user's BYOK; cached responses return without re-billing. Revise dedup does not change billing scope; eval enqueue still follows the multi-part rules in [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval). Credit card service fees ($49 Pass 1, $199 per Pass 2 target) are not triggered by MCP calls; they hit only when the user clicks Analyze on the review screen.

Tool responses include forward-looking guidance directing the assistant's next steps. The `submit_codebase_digest` response advises polling `get_pending_clarifications` at a recommended cadence and responding via `update_codebase_digest` until clarifications are addressed; it also instructs the assistant to tell the user "submitted; you'll get an email when it's ready" without surfacing `draftUrl`. Recommended polling cadences (configurable via admin dashboard): 90 seconds after `submit_codebase_digest` (long initial wait of 10 to 40 minutes); 20 seconds after `update_codebase_digest` or `revise_codebase_digest` (short re-evaluation wait of 30 seconds to 2 minutes). Capable agentic assistants follow the guidance autonomously; less capable ones surface the text for the user to act on.
...
2. Assistant calls `submit_codebase_digest`. Server creates draft (status `evaluating`) and enqueues Quality Evaluator. Response carries `draftUrl` for assistant introspection only; the assistant tells the user "submitted; you'll get an email when it's ready" without surfacing the URL.
3. Assistant polls `get_pending_clarifications` per response guidance and addresses gaps via `update_codebase_digest` while the user waits.
4. When Quality Evaluator (and Pattern & Cluster Analyzer for multi-app digests) completes, the server emails the user with the draft URL.
5. User opens the URL. Review screen shows pre-filled intake steps with "what we observed" annotations, the per-app inventory with quality flags surfaced, the intent-gap checklist.
6. User addresses intent gaps, edits anything that needs editing. Remaining clarifications can be re-delegated to the assistant inline.
7. User clicks Analyze. Pipeline runs.
...
Token format: `agent12_pat_<32 bytes base64url>` (~50 chars total, 256 bits of entropy). The `agent12_pat_` prefix matches the GitHub PAT pattern, making tokens recognizable to secret scanners and reserving namespace for future token types. The MCP server stores SHA-256 hashes in `user_api_tokens.token_hash`; raw tokens are never persisted. Validation parses the `Authorization: Bearer <token>` header, hashes, and queries `user_api_tokens WHERE token_hash = ? AND revoked_at IS NULL`. On match, the request is associated with the matching `user_id` and `last_used_at` is updated (debounced to once per ~60 seconds per token to avoid hot-write per request). On miss, the server returns 401 with a generic error message. Revocation is soft-delete via `revoked_at`; revoked tokens stay in the table for audit. Per-token rate limit defaults to 60 requests per minute (configurable via admin dashboard, enforced via Redis); steady-state polling cadence is well under this ceiling, so the limit only bounds runaway integrations.
...
The MCP server is a dedicated service in the monorepo at `packages/mcp/`, deployed to Railway (or an equivalent long-running host like Fly.io). It runs HTTP+SSE transport per the standard MCP protocol for cloud-hosted servers. We do not deploy it on Vercel because Vercel's serverless function model fits poorly with HTTP+SSE patterns and the longer-lived response shapes some MCP clients use.

Shared dependencies: pulls from `packages/shared/` for the DB client and types, from `packages/agents/` for digest payload Zod schemas, and uses the same Redis instance as `packages/workers/` for BullMQ enqueue. Does not depend on `packages/web/`. Required environment variables: `DATABASE_URL`, `REDIS_URL`, `ENCRYPTION_KEY` (for `tenant_secrets` reads in cost-context lookups), and the auth-validation secret for token hashing.

Local dev adds `pnpm --filter mcp dev` to the existing dev story (Postgres + Redis + Next.js + Workers).
...
Per-app entries marked `isPrivate: true` bypass the global `cv_result_cache`. CV runs fresh per-tool sub-tasks for these on every code-aware run; results are not shared cross-user. This prevents internal corporate apps with colliding names (multiple organizations may each have an app internally named "user-api", "risk-engine", or "audit-trail" representing entirely different products) from leaking into other tenants' lookups.

Public apps surfaced from a code-aware digest (open-source packages with `isPrivate: false`) participate in the global cache normally, keyed by tool name plus version as today (the `cv_result_cache` schema is shared with manifest-tool lookups).
...
Pipeline failures during a code-aware run follow the existing rules in Pipeline failure handling. The richer per-app inventory makes CV's failure handling more important: a CV sub-task failure on an internal app (no manifest fallback available) ships flagged with the digest's source URLs as the manual verification path, the same pattern as for any unavailable data category.
...
For code-aware runs, a seventh specialist prompt fragment (migration-mapping) is invoked with the per-app inventory as additional input. This produces the Migration Mapping section of the spec doc.
...
Runs after the Spec Synthesizer's six (or seven, for code-aware) invocations complete and the spec is mechanically assembled. Takes the assembled spec, verified intake context, the Skeptic's resolved state (including caveat tier assignments), maturity labels per recommended tool, and the active tenant communication context as input. For code-aware runs, the per-app inventory is also passed so the plan can reference specific existing apps per phase. For per-target invocations, the cross-target migration orchestration overview is also passed so per-target plans can reference shared services as prerequisites without re-planning them inside this target's plan.
...
- Completion criteria
- For code-aware runs: explicit migration mapping (which existing apps this phase addresses, what happens to them: replaced / consolidated / retained / deprecated)
...
| Code-Aware Pass 1 | Code-aware intake (codebase analysis via the user's AI assistant) + Pass 1 output: architectural diagram, full CV detail, recommended consolidation summary with target system count, migration orchestration overview (cross-target sequencing and shared-service identification), and Pass 2 pricing math for the identified target set; BYOK required (user's LLM costs billed to user) | $49 / run (BYOK required) |
| Code-Aware Pass 2 | Per spec+plan deliverable: full Pass 2 output for one target system (builder-depth spec sections, phased build plan, migration mapping for which existing apps merge into this target); customer-selectable subset from the consolidation analysis target set; BYOK required | $199 per spec+plan (BYOK required, requires Code-Aware Pass 1; matches text-intake Pass 2 at parity per target) |

**Code-aware pricing model:** Code-aware tiers are priced as service fees on top of the user's BYOK LLM costs. Code-Aware Pass 1 ($49 service fee) produces the architectural assessment, the consolidation analysis with target system count, the migration orchestration overview, and the Pass 2 pricing math for the identified target set. Code-Aware Pass 2 is priced per spec+plan deliverable ($199 each, matching text-intake Pass 2 at parity per target), with customer-selectable subset purchase from the target set surfaced in Pass 1. A 10-target consolidation analysis where the customer purchases all deliverables is $49 (Pass 1) + 10 × $199 (Pass 2) = $2,039 service fees, plus the user's BYOK LLM costs. The customer can also purchase a subset (for example, 3 of 10 targets) and add more later. Failed runs are not billed. The migration orchestration overview is included at no additional cost with any Code-Aware Pass 1 purchase, regardless of how many Pass 2 targets the customer ultimately implements.
...
| `runs` | Run storage and state | id, user_id, status, tier, verified_context (jsonb), verified_context_hash, tenant_context_tag, pass1_output (jsonb), pass2_output (jsonb nullable), maturity_label_distribution (jsonb), charged, parent_run_id (uuid nullable, FK runs.id, for code-aware per-target Pass 2 lineage) |
...
| `codebase_digest_drafts` | Drafts produced by the code-aware intake MCP submission, awaiting user review | id, user_id, tenant_id nullable, digest (jsonb: intake pre-fills, per-app inventory, intent gaps), quality_summary (jsonb: flags, scores, pending clarifications), cluster_analysis (jsonb: Pattern & Cluster Analyzer output), created_at, expires_at, submitted_at nullable |
...
| `manifest_intent_gap_questions` | Curated intent gap question catalog for code-aware intake (admin-managed, evolved via Manifest Gatekeeper) | id, question_id (UNIQUE), question_text, option_type (single-select / multi-select / free-text-only), options (jsonb array), applicable_when (jsonb describing when Copilot should surface this question), confidence_score, last_refreshed_at, vetted, owner |
...
|---|---|---|---|
| Language and runtime | TypeScript throughout (Node.js) | TypeScript backend keeps shared types with the React frontend across workers and agents; Anthropic TS SDK has parity with the Python SDK for this use case. | 2026-04-27 |
| Web framework | Next.js (App Router) | Full-stack TypeScript with React; API routes for quick endpoints (auth, intake, run history); long-running work runs in BullMQ workers, not API routes. | 2026-04-27 |
...
| Prompt caching | Three-layer cache breakpoints per agent, explicit cache_control in every SDK call | Cache reads cost 10% of input tokens, cutting per-run LLM cost 60-90% within TTL. Implementing from day 1 avoids a retrofit across every agent caller. The dashboard cached-token split is the primary cache-health signal. | 2026-04-27 |
| Monorepo structure | packages/ with web/, workers/, agents/, shared/, evals/ | Each package owns a distinct concern: `web/` (Next.js), `workers/` (BullMQ pipelines, no timeout limits), `agents/` (prompt templates + Zod schemas + SDK callers), `shared/` (types + Drizzle), `evals/` (LLM evals, manual trigger only). | 2026-04-27 |
...
| BullMQ wave mapping | BullMQ FlowProducer for the wave structure | FlowProducer handles parallel waves natively, so no custom orchestration is needed. | 2026-04-27 |
| Database indexes | Four composite indexes in the initial migration | Indexing is required before UI for checkpoint reuse, CV cache, run history, and intake filtering. Retrofitting after data accumulates requires a blocking table rewrite. | 2026-04-27 |
...
| Agent caller streaming | callAnthropicAgent switches from client.messages.create() to client.messages.stream(), accumulating input_json_delta chunks and parsing on content_block_stop | Complex user systems produce responses (~6 min in eval) long enough to drop the TCP connection without streaming. Also required for CV progressive disclosure. | 2026-05-01 |
...
| Code-aware terminology — internal app vs user-facing word selection | Internal symbols (code, schemas, prompts, design docs) use `app` as the umbrella name for a digest entry. User-facing rendering (Technical Writer, Spec Synthesizer, Plan Synthesizer, Pattern & Cluster Analyzer, Quality Evaluator clarification questions, MCP response guidance) refers to entries by their `displayName` or `productCategory.displayName`. When an umbrella is needed, the agent selects the most semantically accurate word for the specific entry from per-entry signals (`observedPatterns`, `inputs`/`outputs`, `isPrivate`) — examples: "service" for HTTP-shaped, "worker" for batch, "library" for packaged dependency, "utility" for CLI, "app" for application-shaped. The word "tool" is reserved for manifest tools, T&I architectural "tools," and MCP "tools" and is never used to refer to a digest entry. Screen labels are admin-curated `config.ui.string.*` keys. | "Tool" was overloaded across four senses: digest entries, manifest tools, the T&I architectural concept, and MCP tools. Renaming digest entries to `app` internally and letting agents pick the right word per entry for user-facing surfaces removes the overload; CV terminology stays as-is. | 2026-05-10 |
| Quality evaluator agent | Two-stage. Synchronous server-side normalization, then a background LLM evaluator scores each app and iterates over digest data to increase confidence in low-scoring areas. Missing fields are synthesized rather than surfaced to the user as questions. | Quality of code-aware intake can vary because we don't see source code. | 2026-05-06, updated 2026-05-09 |
| Intent gap question evolution | Curated catalog in `manifest_intent_gap_questions` ([Pre-digest intent collection](#pre-digest-intent-collection)); Manifest Gatekeeper promotes frequently-occurring free-text answers to curated options over time. | Free-text intent is intimidating and produces harder-to-reason-about inputs. Curated options give users cognitive scaffolding and downstream agents cleaner data. Gatekeeper-driven evolution means the catalog gets smarter from real usage rather than from prompt tuning. | 2026-05-06, updated 2026-05-15 |
| Pattern & Cluster Analyzer | Background BullMQ worker, runs after Quality Evaluator. Groups apps by `productCategory`; multi-app categories trigger one LLM call each in parallel, single-app categories skip the LLM. On update, only affected categories re-run. Implementation detail in PLAN.md 3.5b.2. | Consolidation at the product level matches how users think about migration. Sub-component-level would suggest absurdities like "consolidate auth services across products" that nobody wants. | 2026-05-06, updated 2026-05-09 |
| Consolidation analysis Pass 1 report | Consolidation analysis will be omitted if the user did not request it and no opportunity was discovered. Otherwise, consolidation opportunities will be presented as prose or a table, whichever would be clearer. | Consolidation is a load-bearing decision in migration scenarios. Free-tier users see no diagram, so the executive summary must convey any recommendation succinctly and clearly. | 2026-05-06, updated 2026-05-11 |
| Code-aware per-spec+plan pricing | Code-Aware Pass 2 priced per spec+plan deliverable ($199 each, BYOK required), at parity per target with text-intake Pass 2; customer-selectable subset from the consolidation analysis target set. Code-Aware Pass 1 priced at $49/run (BYOK required) and includes consolidation summary, target count, Pass 2 pricing math, and migration orchestration overview. Pass 2 invocations (Spec Synthesizer + Plan Synthesizer) run only for selected targets. | A multi-target run produces N independent spec+plan deliverables, so pricing per run would undercount the work. Pricing each deliverable at $199 matches text-intake Pass 2; code-aware may warrant a premium later once production data validates the difference in value. | 2026-05-07 |
| Provisional pricing | Day-1 prices are provisional and configurable. | Cost and market data will impact future pricing decisions. | 2026-05-07 |
| MCP integration model | User runs an existing IDE assistant (Copilot, Claude Code, Cursor, etc.). We control tool definitions and response payloads; not the assistant's behavior. | MCP is server-responds-on-call; servers can't push. Quality of digest is bounded by quality of assistant. | 2026-05-09 |
| Code-aware digest evaluation completion | MCP `submit_codebase_digest` returns within 1 to 2 seconds with `status: "evaluating"`. Quality Evaluator runs as a background BullMQ worker job. The draft URL is delivered to the user by email when eval completes; submit response carries `draftUrl` for assistant introspection only (see Email-only URL provisioning). | Real per-call latency on existing pipeline agents averages 30 to 160 seconds; large digests (50+ apps) put the evaluator above 5 minutes wall clock even with concurrent caps. Synchronous MCP response would block the user's IDE for too long and risk connection drops. Async + email is consistent with the pipeline run completion model already in place. | 2026-05-09, updated 2026-05-16 |
| Email-only URL provisioning | Draft URL is not surfaced to the user via the assistant chat. Submit response includes `draftUrl` for assistant introspection (state polling, autonomous clarification loop); assistant response guidance directs it to tell the user "submitted; you'll get an email when it's ready." User's first sight of the URL is the completion email. | A partial-state review screen (opened before Quality Evaluator finishes) confuses users: they see empty scores and a clarification loop happening in the background. Gating URL visibility on eval completion delivers a fully-loaded review surface and matches the async submission UX already established for text-intake runs. | 2026-05-16 |
| BYOK cost transparency | Public pre-auth pricing page with per-app token usage ranges and per-provider rates, "effective as of" date stamped. `estimate_digest_cost` MCP tool returns dynamic estimates. No cost gate at submit. | Users paying BYOK should see the math before committing. Static page covers occasional users; the endpoint covers integrated assistants. The user's own API account bounds the spend, so no gate is needed. | 2026-05-09 |
...
| Background eval concurrency | Queue with coalescing. Eval start is gated by [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval); once started, in-flight jobs run to completion and are not canceled. The server applies new create / update / delete to draft state immediately and registers a pending follow-up eval scope. Multiple in-flight-window calls UNION into one coalesced follow-up; one email when all pending evals complete. | Cancellation wastes user BYOK on partially-completed work; queue lets started work finish cheaply. Coalescing prevents the unbounded-queue failure mode (5 rapid updates produce 1 follow-up, not 5). | 2026-05-12, updated 2026-05-16 |
...
| Empty update call behavior | Empty bodies return `isError: true` unless resolved `final: true` carries purpose on `revise_codebase_digest`. Per-tool envelope rules carry the exact contract. | MCP has a dedicated `ping` method (modelcontextprotocol.io/specification/2025-11-25/basic/utilities/ping) for connectivity verification, so empty bodies without intent are not legitimate ping traffic. The default-true `final` makes the empty multi-part finalization call the dominant legitimate empty body. | 2026-05-12, updated 2026-05-16 |
...
| Idempotency for write tools | Two-tier dedup (explicit `idempotencyKey` plus implicit hash) on `submit_codebase_digest` and `revise_codebase_digest`; revise scoped by `draftId`. Contract lives in the Idempotency subsection. | Naive retries paying double BYOK is the failure mode; implicit hash dedup is rounding-error cheap. Multi-part submission makes the protection load-bearing on revise (each part is billable, and a network retry can silently double-create an app entry). | 2026-05-14, updated 2026-05-16 |
...
| Outcome-gated execution | Pipeline work runs only when its outcome would shift the recommendation, at both whole-work granularity (catalog inclusion, surfaced gaps, iteration loops, retries, supplementary research) and within-work granularity (specific clarifying questions to attempt, specific cycles to run). Existing instances: Skeptic Advisory-floor early-exit, CV result cache, Pattern & Cluster Analyzer single-app short-circuit, Quality Evaluator sufficiency threshold + filtered iteration, intent-gap catalog discipline. | Speculative work pays token and latency cost without shifting the recommendation; naming the pattern lets new instances reference rather than re-derive. | 2026-05-15 |
| Pre-digest intent collection | Two structured questions (target topology, timeline pressure) plus optional free-text constraints, collected before the digest ([Pre-digest intent collection](#pre-digest-intent-collection)). Bimodal: web UI (sign in, answer, receive assistant snippet referencing the session) or MCP payload (assistant collects in chat, ships as optional fields on `submit_codebase_digest`). Both paths unify into the same draft state. Catalog lives in `manifest_intent_gap_questions`. | Digest production is pure inventory; intent does not affect what the assistant emits, so collecting it earlier keeps the review screen focused on validating digest accuracy and lets Quality Evaluator and Pattern & Cluster Analyzer reason with intent during background passes. Two questions captures topology + timeline; consolidation grouping comes from Pattern & Cluster Analyzer post-digest. Two paths support both planning-surface and IDE-native users. | 2026-05-15 |
| Agent slot + variant abstraction | Registry uses slot + variant structure; variant key is a flat string (`orchestration.textIntake`, `orchestration.codeAwareIntake`) with dots / underscores as naming convention only. Runner resolves variant per run via a resolver function consuming `runContext` (intake mode, user id, tenant id, tier); default resolver dispatches by intake mode. Variant selection is config-driven per tenant via the standard `config` table (`owner = 'global'` default, `owner = tenant_id` override). MVP ships only `textIntake` variants; code-aware runs use `textIntake` until specific agents are forked. Forking later: add registry entry, write prompt, build eval, extend resolver. Implementation in PLAN.md 3.5c. | Without the abstraction, forking later costs ~2-3x because flat keys are baked into Runner, FlowProducer, checkpoint reuse, and observability; ~3-4 days now buys per-agent forkability plus resolver extensibility (A/B, tier, tenant) without further refactor. | 2026-05-15 |
| Per-app inferential field expansion | `AppEntry` gains two inferential fields beyond identification + interface metadata: `currentDecisionMaking` (where judgment lives today) and `humanInTheLoop` (existing HITL surfaces). Both feed the Wave 1+ agentic-upgrade-opportunity surface (with `humanInTheLoop` also serving as Trust & Control's direct input). Spec carries a one-line example output per inferential field including `primaryPurpose` and `distinguishingCharacteristics`. | Tool manifests plus interfaces alone don't expose where in each app the agentic upgrade should land. The other three candidates (`stateAndMemory`, `dataSensitivity`, `failureModes`) were dropped because they are partially derivable from `dependencies`, `observedPatterns`, `externalIntegrations`, and `isPrivate`, and the marginal Wave 1+ signal did not justify the per-app cognitive load plus assistant-variance cost. Example outputs reduce assistant variance on the two fields with no code-derivable anchor. | 2026-05-16 (initial five-field design rolled back same day after pushback pass) |
| Multi-part submission and deferred eval | `final` defaults to `true`; multi-part assistants opt out via explicit `final: false` on the initial submit and intermediate revises. Eval defers until resolved `final: true` or the idle-timeout backstop. See [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval) for flow and rationale. | Default-true keeps single-shot friction at zero. Per-app scoring needs full L2 group context; consolidation analysis needs the complete inventory. Firing per part costs 60-80% extra tokens and produces unstable intermediate scores. | 2026-05-16 (default-true reaffirmed same day after pushback pass) |
| Per-target Pass 2 invocation model | One Pass 2 per target system, independently selected/billed/delivered. Each per-target run re-pipelines Waves 1-3 + Skeptic on live digest filtered to target's current app membership, then renders the deliverable (six architecture spec sections + Migration Mapping + phased plan). User sees "Pass 2 running." Implementation in PLAN.md 3.5b.5. | Per-target reasoning produces sharper recommendations than per-target rendering of shared upstream; one-at-a-time purchase matches user mental model and avoids batched-checkout complexity. | 2026-05-18 |
| Pattern & Cluster Analyzer bypass on per-target re-pipeline | PCA does not re-run during per-target Pass 2 re-pipeline. Per-target Waves 1-3 receive parent's PCA output filtered to subset categories as context. | User confirmed the consolidation grouping at parent Pass 1; re-running PCA on the subset risks contradicting the already-confirmed verdict (subset clustering can differ from full-inventory clustering). | 2026-05-18 |
| Cross-target overview auto-refresh | Parent Pass 1's migration orchestration overview is used as cross-target context. If digest has drifted in ways affecting cross-target dependency boundaries (additions, or removals that flip cross-target dependency status), the overview auto-refreshes before re-pipeline runs (one Technical Writer-equivalent LLM call, BYOK, no service fee). Refreshed overview persists alongside the original; parent Pass 1 deliverable stays as historical artifact. | Stale cross-target context degrades per-target deliverable quality; charging a separate fee feels nickel-and-dime against the positioning that users should be elated at the value. | 2026-05-18 |
| Live target list for Pass 2 buy affordances | Pass 2 buy buttons reflect current target list (derived from current PCA output + user strategy answers). Parent Pass 1 deliverable stays as historical artifact. | User revisions to the digest must be respected; ignoring them would deliver analysis contradicting explicit user intent. | 2026-05-18 |
