...
| Tool | Purpose |
|---|---|
| `submit_codebase_digest` | Creates a new draft. Accepts the digest payload (per-app inventory, optional intake pre-fills, optional raw package manifests). Pre-digest intent ([Pre-digest intent collection](#pre-digest-intent-collection)) reaches this tool either as inline fields on the payload (MCP-payload path) or by reference to a web-UI-created session (web-UI path); the I/O contract subsection below predates the pre-digest design and is updated in a follow-up PR. Server normalizes synchronously and returns draft URL plus `status: "evaluating"`. Quality Evaluator runs as a background job; email on completion. |
...
- `submit_codebase_digest`. **Input:** `digest` (containing `appsToCreate` as array of `AppEntryCreate`, optional `intentGapsToCreate` as array of `IntentGapCreate`, optional `intakePrefills` as `IntakePrefills`) plus optional `idempotencyKey` (see Idempotency subsection below). **Output:** `draftId`, `draftUrl`, `status` set to `evaluating`, `assignedAppIds`, `assignedIntentGapIds`, `expiresAt` (iso8601), `pollGuidance`, `dedupedFromRecentSubmit` (boolean), `dedupePath` (enum: `explicit`, `implicit`, or null when not deduped), `isError`, `partialFailures`.
