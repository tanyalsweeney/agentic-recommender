...
3. *Intent gaps.* An explicit checklist of what the code couldn't tell us. Each gap is a structured prompt with three parts: a question, a curated set of common-answer options (check-all-that-apply where multiple answers are valid, single-select where exactly one answer is valid), and an optional free-text field for cases where the user's answer doesn't fit the curated options. Examples:
   ...
   - "What is the target consolidation goal?" with options [single unified service, smaller set of focused services, replace one specific tool, modernization without consolidation] plus free text
...
The review screen surfaces a small badge next to each dependency in the per-tool inventory:
...
| `clusters` | array of `{label, description, toolIds}` | Within-category groupings |
...
| `blockers` | array of `{type, description, affectedToolIds}` | Identified blockers (compliance / contractual / technical); empty if none |
| `supportingEvidence` | array of `{toolIds, claim}` | Pointers from reasoning back to specific entries |
...
| `consolidationStrategyQuestion` | intent-gap-format question with fixed options | The user-intent meta-question per multi-tool category |
...
| Pattern & Cluster Analyzer | Dedicated agent for code-aware digests, runs sequentially after Quality Evaluator as a background BullMQ worker job. Groups tools by `productCategory`, makes one LLM call per multi-tool category in parallel (single-tool categories skip the LLM call and emit `opportunity: 'none'` deterministically). 3-layer prompt cache (system + rubric, full per-app inventory + tenant context, per-category subset). Output: `consolidationOpportunity`, structured `clusters` / `blockers` / `capabilityVariance` / `supportingEvidence`, `clarificationQuestions` in intent-gap format, and the `consolidationStrategyQuestion` per multi-tool category. Stored in dedicated `cluster_analysis` jsonb column on `codebase_digest_drafts`. On update, only affected categories re-run. | Cluster analysis at the `productCategory` (tool/product) level matches how users think about migration: consolidate tools that do similar things. Sub-component-level analysis would suggest absurdities like "consolidate auth services across products" that nobody actually wants. Targeted clarification surfaces context that would not otherwise be captured (regulatory, performance, contractual reasons that drove divergent choices). Per-category LLM calls in parallel (skipping single-tool) keeps cost economic; sequential placement after Quality Evaluator means the analyzer sees synthesized fields. Always running the analysis is cheap; surfacing depends on user intent and assessment magnitude. | 2026-05-06, updated 2026-05-09 |
