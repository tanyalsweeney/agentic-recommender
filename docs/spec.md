...
| Code-aware digest implicit dedup window | 5 minutes | Window within which a hash-matching submit from the same user with no `idempotencyKey` is auto-deduped. Per-tenant configurable. |
...
1. *Per-app inventory.* One structured entry per discovered app. For purposes of the digest, an app is a discrete product or application within the codebase (e.g., FinanceLaunchRisk, ContractVolumePredictor), not an internal sub-component (an auth-service inside an app, a data access layer). Internal sub-components are described as part of the app's entry (in `dependencies` and `distinguishingCharacteristics`), not as separate entries. The migration unit is the app, not the sub-component. Each entry carries:
   ...
   - `primaryPurpose`: concrete description of what THIS app does ("evaluates upcoming product launches against historical launch profiles to surface risk patterns observed in similar past launches"), not a category label ("handles risk evaluation").
   - `language`, `framework`.
   ...
   - `currentDecisionMaking`: where judgment, scoring, or threshold-branching happens in this app today (scoring functions, decision trees, manual approval workflows, threshold-based logic). The agentic-upgrade opportunity per app; load-bearing for Wave 1+ recommendations about where to add agency.
   - `humanInTheLoop`: where humans currently gate, approve, or verify in this app's flow (approval queues, manual triage hooks, scheduled human review steps). Trust & Control agent's input for HITL recommendations.
   - `stateAndMemory`: what state this app holds (session-scoped, persistent, shared across apps). Memory & State agent's input.
   - `dataSensitivity`: kinds of data flowing through (PII, financial, regulated, internal-only, public). Security agent's input.
   - `failureModes`: how this app currently fails and is recovered (retries, dead-letter queues, manual triage, fail-open vs fail-closed). Failure & Observability agent's input.
...
Large or richly-detailed digests routinely exceed what an assistant can produce in a single pass. The assistant is encouraged to pace submission across multiple MCP calls when the inventory has many apps (typically >10), per-app analysis is rich (capturing all inferential fields including `currentDecisionMaking` and `humanInTheLoop` at depth), or assistant context-window pressure favors splitting.

*Flow:*

1. Initial `submit_codebase_digest` creates the draft with `status: "intake-in-progress"`. Server merges the payload into draft state immediately. Quality Evaluator and Pattern & Cluster Analyzer are not started.
2. Subsequent parts arrive via `revise_codebase_digest` with `apps.create` for new entries (or `apps.update` for refinements to previously-submitted apps). Server applies changes immediately. No eval is triggered.
3. The assistant signals intake completion with `revise_codebase_digest` carrying `final: true`. Server flips status to `evaluating` and enqueues Quality Evaluator and Pattern & Cluster Analyzer against the complete final state.
4. *Idle-timeout backstop.* If no `revise_codebase_digest` call (content-bearing, `final: true`, or `triggerEvalNow`) arrives within `multi_part_idle_timeout_minutes` (default 30, admin-configurable), the server flips status to `evaluating` automatically and enqueues eval. The completion email notes that intake may have been cut short.

*Opt-in mid-stream eval (`triggerEvalNow`).*

The assistant can include `triggerEvalNow: true` on any `revise_codebase_digest` call to enqueue Quality Evaluator immediately against the current draft state without flipping to final. Use case: the assistant wants per-app eval feedback for already-submitted apps before composing more parts (drives the clarification loop earlier rather than waiting for `final`).

- Server enqueues Quality Evaluator against the current per-app inventory. Pattern & Cluster Analyzer does NOT run mid-stream; its value is in the complete picture and partial-inventory cluster analysis is misleading.
- Status stays `intake-in-progress`. The assistant can continue submitting more parts during or after the mid-stream eval.
- Existing coalescing applies: if more `revise_codebase_digest` calls arrive while mid-stream eval is running, the new parts queue as follow-up eval per Background eval concurrency rules.
- Clarification loop fires normally for entries the mid-stream eval flags: `get_pending_clarifications` returns the gaps; assistant responds via `update_codebase_digest`.
- BYOK billed per mid-stream eval pass; the user opted in via the flag.

*Why defer eval by default.*

Quality Evaluator's per-app scoring relies on the L2 group context (full per-app inventory) for synthesis and peer comparison; Pattern & Cluster Analyzer needs the complete inventory to identify clusters and consolidation opportunities. Eval against partial inventory produces unstable scores and misleading consolidation analysis. Token savings on partial-submission flows are estimated at 60-80% versus firing eval per part.
...
| Tool | Purpose |
|---|---|
| `submit_codebase_digest` | Creates a new draft. Accepts the digest payload (per-app inventory, optional intake pre-fills, optional raw package manifests). Pre-digest intent ([Pre-digest intent collection](#pre-digest-intent-collection)) reaches this tool either as inline fields on the payload (MCP-payload path) or by reference to a web-UI-created session (web-UI path); the I/O contract subsection below predates the pre-digest design and is updated in a follow-up PR. Optional `final: true` on input signals single-shot intake (server flips status to `evaluating` immediately); absent or false, status returns as `intake-in-progress` pending subsequent `revise_codebase_digest` with `final: true` or the idle-timeout backstop. See [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval). |
...
| `revise_codebase_digest` | Assistant-driven path. Assistant-initiated changes to apps, intent gaps, or intake prefills (new app discovery, displayName fixes, recategorization, freeform edits). Per-section `create` / `update` (partial-merge) / `delete` on apps and intent gaps; partial-merge on intake prefills. Also carries `final: true` (intake complete, enqueues full eval) and optional `triggerEvalNow: true` (enqueues Quality Evaluator mid-stream without flipping to final; Pattern & Cluster Analyzer still defers to `final`). See [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval). |
...
- `submit_codebase_digest`. **Input:** `digest` (containing `appsToCreate` as array of `AppEntryCreate`, optional `intentGapsToCreate` as array of `IntentGapCreate`, optional `intakePrefills` as `IntakePrefills`), optional `final: true` (single-shot intake signal), plus optional `idempotencyKey` (see Idempotency subsection below). **Output:** `draftId`, `draftUrl`, `status` enum: `intake-in-progress` (default) or `evaluating` (when `final: true` on input), `assignedAppIds`, `assignedIntentGapIds`, `expiresAt` (iso8601), `pollGuidance`, `dedupedFromRecentSubmit` (boolean), `dedupePath` (enum: `explicit`, `implicit`, or null when not deduped), `isError`, `partialFailures`.
...
- `revise_codebase_digest`. **Input:** `draftId`, optional `apps.{create, update, delete}`, optional `intentGaps.{create, update, delete}`, optional `intakePrefills` (partial-merge), optional `final: true` (intake-complete signal; flips status to `evaluating` and enqueues full eval), optional `triggerEvalNow: true` (enqueues Quality Evaluator mid-stream without flipping to final; ignored when `final: true` is also set). Empty bodies with only `final: true` or `triggerEvalNow: true` are valid and do not return `isError`. **Output:** `assignedAppIds`, `assignedIntentGapIds`, `affectedEntries`, `affectedCategories`, `status` enum: `intake-in-progress` or `evaluating`, `danglingReferences` (array of `{id, displayName, referencePath}`, advisory per existing referential integrity decision), `pollGuidance`, `isError` (true if no operation and no signal flag landed), `partialFailures`.
...
| `stateAndMemory` | string or null | What state this app holds (session, persistent, shared across apps) |
| `dataSensitivity` | string or null | Kinds of data flowing through (PII, financial, regulated, public, internal) |
| `failureModes` | string or null | How this app currently fails and is recovered (retries, DLQ, manual triage) |
...
Two-tier dedup, mode determined by whether the assistant supplies an `idempotencyKey`. Both tiers compute the same `request_hash`: deterministic JSON canonicalization of the input payload followed by SHA-256. Hash compute and Redis storage are both rounding error at expected scales.

*Tier 1: explicit (assistant supplies `idempotencyKey`).* Server stores `{user_id, idempotencyKey} → {request_hash, response}` in Redis with a 24-hour TTL.

- Same key + identical payload: server returns the cached response with `dedupedFromRecentSubmit: true` and `dedupePath: "explicit"`. No new draft, no new background evaluation, no additional BYOK billing.
- Same key + different payload: server returns a `partialFailures` entry with `code: "idempotency_collision"` and `isError: true`. The error message instructs the assistant to generate a fresh key for a new intentional submit. Indicates an assistant key-management bug, not user intent.
- Fresh key (no prior entry for this `{user_id, idempotencyKey}`): server processes the call as a new submit and stores the entry. Tier 2 is bypassed on keyed submits (no implicit cache read or write); the explicit key is the assistant's signal that it wants precise retry semantics.

*Tier 2: implicit (assistant supplies no `idempotencyKey`).* Server checks `{user_id, request_hash} → response` in Redis with a short TTL (`codebase_digest_drafts.implicit_dedup_window_minutes`, default 5, per-tenant configurable).

- Payload hash matches a recent submit by the same user within the window: server returns the cached response with `dedupedFromRecentSubmit: true` and `dedupePath: "implicit"`. Catches the dominant naive-retry failure mode (assistant retries after a network blip without supplying a key) at essentially zero infrastructure cost.
- No match: server processes the call as a new submit and stores the implicit cache entry.

*Override path for forcing a new run.* To re-evaluate an identical payload within the implicit window (intentional A/B re-eval, force-refresh, etc.), the assistant supplies a fresh `idempotencyKey`. This switches into Tier 1 where the fresh key has no prior entry, the call processes normally as a new submit, and the implicit cache is not consulted. Explicit always beats implicit.

Implemented as a Redis-backed helper in `packages/mcp/src/idempotency.ts`. Tool description language recommends assistants generate a fresh uuidv7 per user-initiated submit, cache it locally during the call, and reuse the cached key only on retries of the same call. Assistants that follow this guidance get precise Tier 1 semantics; assistants that skip the key still get Tier 2 protection from accidental duplicate-bill failures.

Each successful submit triggers a fresh background evaluation billed against the user's BYOK. Cached responses from either tier return without re-billing. New user-initiated submits, whether for an updated codebase or a forced fresh look at an identical one (via a fresh `idempotencyKey`), are billed as separate evaluations. Credit card service fees ($49 Pass 1, $199 per Pass 2 target) are not triggered by MCP submit; they hit only when the user clicks Analyze on the review screen.
...
5. Review screen shows: pre-filled intake steps with "what we observed" annotations, the per-app inventory with quality flags surfaced, the intent-gap checklist.
...
| Code-aware digest evaluation completion | MCP `submit_codebase_digest` returns within 1 to 2 seconds with draft URL plus `status: "evaluating"`. Quality Evaluator runs as a background BullMQ worker job. Email notifies the user on completion. | Real per-call latency on existing pipeline agents averages 30 to 160 seconds; large digests (50+ apps) put the evaluator above 5 minutes wall clock even with concurrent caps. Synchronous MCP response would block the user's IDE for too long and risk connection drops. Async + email is consistent with the pipeline run completion model already in place. | 2026-05-09 |
...
| Background eval concurrency | Queue with coalescing. Quality Evaluator and Pattern & Cluster Analyzer jobs do not start until `final: true` arrives via `submit_codebase_digest` or `revise_codebase_digest` (or the idle-timeout backstop fires); see [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval). Once started, in-flight jobs run to completion and are not canceled when new MCP calls arrive. The server applies any new create / update / delete to draft state immediately (incremental, not blocked) and registers a pending follow-up eval scope. If multiple MCP calls land while an eval is still running, their changes UNION into one pending follow-up. When the in-flight eval completes, the server enqueues the coalesced follow-up against the latest state. One email when all pending evals complete. | Cancellation wastes user BYOK on partially-completed work; queue lets started work finish cheaply. Coalescing prevents the unbounded-queue failure mode (5 rapid updates produce 1 follow-up, not 5). Deferring eval until `final: true` avoids wasting tokens on partial-inventory scoring (Quality Evaluator's L2 group context grows as parts arrive) and misleading partial-inventory consolidation analysis (Pattern & Cluster Analyzer). | 2026-05-12, updated 2026-05-16 |
...
| Empty update call behavior | Strict for `update_codebase_digest`: all sections empty or absent returns HTTP 200 with `isError: true` and content text directing the assistant to use the MCP `ping` method for connectivity checks or include at least one operation. For `revise_codebase_digest`: empty body is valid IFF accompanied by `final: true` or `triggerEvalNow: true` (the signal flags are intentional empty-body calls). Otherwise the same strict rule applies; no background jobs enqueued. | MCP has a dedicated `ping` method at the protocol level (modelcontextprotocol.io/specification/2025-11-25/basic/utilities/ping) for connectivity verification, so empty updates without intent are not legitimate ping traffic. The `final: true` and `triggerEvalNow: true` signal flags are first-class purposes for an otherwise-empty revise call. | 2026-05-12, updated 2026-05-16 |
...
| Idempotency for `submit_codebase_digest` | Two-tier dedup. Tier 1 (explicit, 24h TTL): when assistant supplies `idempotencyKey`, server stores `{user_id, key} → {request_hash, response}`. Same key + same payload returns cached (no re-billing); same key + different payload returns `idempotency_collision`. Tier 2 (implicit, 5-min default TTL, per-tenant configurable): when no key is supplied, server stores `{user_id, request_hash} → response`. Hash-matching same-user submit inside the window returns cached with `dedupePath: "implicit"`. Explicit beats implicit; supplying a fresh key bypasses Tier 2. | Stripe-style explicit dedup alone leaves naive retries paying double BYOK; adding implicit hash dedup costs essentially nothing (hash and Redis lookup are rounding error) and protects users from sloppy assistants while preserving the override path (fresh key forces a new run). Server cannot pre-issue keys because MCP servers can't push to clients, so client-supplied keys remain the only working option for Tier 1. | 2026-05-14 |
...
| Per-app inferential field expansion | `AppEntry` gains five inferential fields beyond identification + interface metadata: `currentDecisionMaking` (where judgment lives today), `humanInTheLoop` (existing HITL surfaces), `stateAndMemory` (held state shape), `dataSensitivity` (data classes), `failureModes` (current failure + recovery). Each has a clear downstream consumer (Wave 1+ agents for the agentic-upgrade-opportunity surface; T&C, Memory & State, Security, F&O for the per-domain inputs). | Tool manifests plus interfaces alone don't expose where in each app the agentic upgrade should land; downstream agents would have to reverse-engineer from indirect signals at 85% accuracy bar. Explicit fields move the signal from inference-from-context to direct-input, and the assistant has unique extraction advantage (code access) over the user on the review screen. | 2026-05-16 |
| Multi-part submission and deferred eval | Assistants pace large or deep digests across multiple MCP calls: initial `submit_codebase_digest` plus subsequent `revise_codebase_digest` parts plus a final `revise_codebase_digest` with `final: true`. Draft status starts as `intake-in-progress`; flips to `evaluating` on `final: true` (or idle timeout, default 30 min admin-configurable). Quality Evaluator and Pattern & Cluster Analyzer defer to `final` by default. Opt-in mid-stream Quality Evaluator via `triggerEvalNow: true` on `revise_codebase_digest`; Pattern & Cluster Analyzer always defers regardless of `triggerEvalNow`. See [Multi-part submission and deferred eval](#multi-part-submission-and-deferred-eval). | Per-app scoring needs full L2 group context to be stable; consolidation analysis needs the full inventory to be meaningful. Firing eval per part wastes 60-80% of tokens and produces unstable intermediate scores the user later has to reconcile. Single-shot assistants use `final: true` on submit to avoid the 30-min idle wait; multi-part assistants use the explicit signal flow. | 2026-05-16 |
